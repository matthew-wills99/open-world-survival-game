        ...
        for(int y = 0; y < dimensions; y++)
        {
            for(int x = 0; x < dimensions; x++)
            {
                ...
                else
                {
                    ...
                    if(y > 0)
                    {
                        Cell up = gridComponents[x + (y - 1) * dimensions].GetComponent<Cell>();
                        List<Tile> validOptions = new List<Tile>();
                        
                        foreach(Tile possibleOptions in up.tileOptions)
                        {
                            /*
                            this is where the valid options for the given tile are found, so far 
                            it only gets the ones manually listed in the upNeighbours array of tiles and does not 
                            yet account for the boolean variables in Tile.cs
                            */
